...
```bash
docker pull ghga/state-management-service:4.3.2
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/state-management-service:4.3.2 .
```
...
docker run -p 8080:8080 ghga/state-management-service:4.3.2 --help
```
